...
func NewCPU(mmu *MMU.MMU, wg *sync.WaitGroup) *CPU {
	return &CPU{
		mmu:   mmu,
...
func (c *CPU) Run() {
	...
	if PerfLogging {
		var amountInstructions int64 = 0
		tt := time.Now()
		for !c.shouldHalt {
			Logger.Debugf("CPU Clock Tick")
			amountInstructions++
			if amountInstructions == 1000000 {
				tn := time.Now()
				diffNano := tn.Sub(tt).Milliseconds()
				// Calculate the amount of instructions per second
				freq := float64(amountInstructions*100) / float64(diffNano)
				Logger.Infof("Frequency: %f instructions per second", freq)
				amountInstructions = 0
				tt = tn
			}
		...
		for !c.shouldHalt {
			if c.ps.brk {
				Logger.Debugf(c.ToString())
				Logger.LogDebugInstruction(c.pc)
				cmd := ""
				_, _ = fmt.Scanln(&cmd)
